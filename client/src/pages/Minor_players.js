...
function MajorPlayer({
  ...
  return (
    <div className="minor_num">
      <p className="minor_num_p">{NAME}</p>
      <img src={IMAGE} alt={NAME} />
      <div>
        <table className="minor_num_table">
          <tbody>
            <tr className="minor_num_tr">
              <th>NO</th>
              <td>{BACKNUM}</td>
            </tr>
            <tr className="minor_num_tr">
...
function MinorPlayers() {
  const [player] = useFetch("http://localhost:5000/api/players/2");

  return (
    <div className="minor_player">
      <div className="minor_player_box">
        <h3>선수단 소개 (2군)</h3>
        {player &&
          player.map((item) =>
